...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 44) and (File.idSample = Sample.id) ')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 44 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) ')
...
temp = sapply(cvQueries, function(x){
  ...
  in.r1 = paste0(cvInput, 'trim_', f)
  #in.r2 = paste0(cvInput, 'trim_', lf[[2]])
  out.sam = paste0(cvOutput, f, '.sam')
  p1 = paste(in.r1, out.sam, sep=' ')
...
p1 = paste('hisat2 --trim5 10 -x', cvGeneIndex, '-U', '$inr1', '-S', '$outsam', sep=' ')
com = paste(p1)
