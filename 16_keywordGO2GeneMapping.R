...
load(file='results/fit.stan.nb_28Jan.rds')
...
dfData$Coef.2 = factor(dfData$fReplicates:dfData$ind)
str(dfData)
...
dfGO = AnnotationDbi::select(org.Mm.eg.db, keys = 'Nox4', columns = c('GO'), keytype = 'SYMBOL')
...
dim(dfGO)
## keyword search
i = grep('reduction', dfGO$DEFINITION, ignore.case = T)
length(i)
temp = dfGO[i,]

i = grep('oxidation', dfGO$DEFINITION, ignore.case = T)
length(i)
...
head(d.bk)
d.bk$coef = colMeans(mCoef[,d.bk$cols])
d.bk$differentiated = factor(d.bk$fBatch, levels=c('WT', 'Tg'))

library(lattice)
xyplot(coef ~ differentiated | SYMBOL, data=d.bk, type=c('l', 'p'), scales=list(relation='free', x=list(cex=0.7), y=list(cex=0.7)), 
       ylab='Model Estimated log Average', main=list(label='profile of genes sharing GO terms with Nox4', cex=0.8),
       xlab='Condition')
