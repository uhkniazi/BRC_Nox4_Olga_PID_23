...
q = paste0('select count(File.idSample) as files, Sample.idData, Sample.title, Sample.id as SampleID from File, Sample
           where (Sample.idData = 44 and File.idSample = Sample.id) group by File.idSample')
dfQuery = dbGetQuery(db, q)
dfQuery$title = gsub(" ", "", dfQuery$title, fixed = T)
dfQuery
# get the count of files
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 44) and (File.idSample = Sample.id) and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 44 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   (File.type = "fastq")')
...
cvArrayJob = paste0('#$ -t 1-', nrow(dfCounts))
...
temp = lapply(cvQueries, function(x){
  ...
  in.s1 = paste0(cvInput, f, '.sam')
  # output file in bam format
  s2 = paste0(cvInput, f, '.bam')
  # remove low quality reads below 10
  s3 = paste0(cvInput, f, '_q10.bam')
  # sort the file
  s4 = paste0(cvInput, f, '_q10_sort.bam')
  # remove duplicates
  s5 = paste0(cvInput, f, '_q10_sort_rd.bam')
...
p1 = paste('samtools rmdup -s', '$bamq10sort', '$bamrd', sep=' ')
com4 = paste(p1)
# create index
