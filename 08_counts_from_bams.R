...
where (Sample.idData = 44) AND (File.idSample = Sample.id AND File.type like "%duplicates removed%")')
...
lCounts = lapply(lFiles, function(bfl){
  ...
  return(assays(summarizeOverlaps(oGRLgenes, oBamFiles, ignore.strand = F, singleEnd=T))$counts)
})

identical(names(lCounts), dfSample$title)
names(lCounts) = dfSample$sid

## save the summarized experiment object
setwd(gcswd)
n = make.names(paste('lCounts rd did 44 olga rds'))
...
#                 comment='list of Count matrix from olga mouse nox4 project with quality 10 and duplicates removed')
