...
l = tapply(d$cols, d$split, FUN = function(x, base='WT', deflection='Tg') {
  c = x
...
## produce the plots 
f_plotVolcano(dfResults, 'Tg vs WT')#, fc.lim=c(-2.5, 2.5))
f_plotVolcano(dfResults, 'Tg vs WT', fc.lim=range(dfResults$logFC))

m = tapply(dfData$values, dfData$ind, mean)
i = match(rownames(dfResults), names(m))
m = m[i]
identical(names(m), rownames(dfResults))
plotMeanFC(log(m), dfResults, 0.01, 'Tg vs WT')
table(dfResults$adj.P.Val < 0.1)
# FALSE  TRUE 
# 3435   120 
## save the results 
write.csv(dfResults, file='results/DEAnalysisTgVsWT.xls')
...
plotDispEsts(oDseq)
oRes = results(oDseq, contrast = c('Treatment', 'Tg', 'WT'))
plotMA(oRes)
